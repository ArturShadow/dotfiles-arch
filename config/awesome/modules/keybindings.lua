...
globalkeys =
    gears.table.join(
        ...
        -- Custom bindings
        awful.key({ alt }, "p", function() awful.spawn.with_shell("~/.local/bin/rofi-powermenu.sh") end,
            { description = "Open power menu grid", group = "launcher" }),
        awful.key({ alt }, "space", function() awful.spawn("rofi -modi drun -show drun") end,
            { description = "Open rofi on drun mode", group = "launcher" }),
        awful.key({ alt, "Shift" }, "l", function() awful.spawn("betterlockscreen -l") end,
            { description = "Lock screen", group = "launcher" })
    )

-- Tags 1-9 keybindings con local index
for i = 1, 9 do
    local index = i
...
clientbuttons = gears.table.join(
    ...
    awful.button({ modkey }, 1,
        function(c)
            c:emit_signal("request::activate", "mouse_click", { raise = true }); awful.mouse.client.move(c)
        end),
    awful.button({ modkey }, 3,
        function(c)
            c:emit_signal("request::activate", "mouse_click", { raise = true }); awful.mouse.client.resize(c)
        end)
)
